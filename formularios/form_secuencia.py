...
from config import  COLOR_BARRA_SUPERIOR, COLOR_BORDE, COLOR_BOTON_SECUNDARIO, COLOR_BOTON_SECUNDARIO_HOVER, COLOR_GRIS_MEDIO, COLOR_TEXTO, COLOR_TEXTO_SECUNDARIO, COLOR_VERDE_HOVER
import threading
...
class FormularioSecuencia():
    ...
    def preparar_area_imagenes(self):
        ...
        # Frame para contener la colección de imágenes
        self.frame_imagenes = ctk.CTkFrame(
            self.contenedor_elementos_carga,
            fg_color="transparent"
        )
        # No empaquetamos aún para mantenerlo oculto
        
        # Crear grid para mostrar las imágenes en filas y columnas
        self.grid_imagenes = ctk.CTkFrame(
            self.frame_imagenes,
            fg_color="transparent"
        )
        self.grid_imagenes.pack(fill="both", expand=True, padx=10, pady=10)
        ...
        self.boton_agregar.pack(side="left", padx=(0, 10))
        
        # Botón para eliminar todas las imágenes
        self.boton_eliminar_todas = ctk.CTkButton(
            self.frame_botones,
            text="Eliminar Todas",
            font=("Segoe UI", 13),
            fg_color="#f39200",
        ...
        self.boton_eliminar_todas.pack(side="left")
        
        # Botón para continuar (reemplaza al botón "Procesar Imágenes")
    ...
    def mostrar_grid_imagenes(self):
        """Muestra las imágenes cargadas en un grid"""
        for widget in self.grid_imagenes.winfo_children():
            widget.destroy()
        columnas = 3
        total_imagenes = len(self.imagenes_originales)
        for i, imagen_previa in enumerate(self.imagenes_originales):
            fila = i // columnas
            columna = i % columnas
            frame_imagen = ctk.CTkFrame(
                self.grid_imagenes,
                fg_color=self.color_gris_medio,
                corner_radius=10,
                width=200,
                height=200
            )
            frame_imagen.grid(row=fila, column=columna, padx=10, pady=10, sticky="nsew")
            ...
            label_imagen.pack(expand=True, fill="both", padx=5, pady=5)
    ...
    def mostrar_error(self, mensaje):
        ...
        ventana.title("Error")
        ventana.iconbitmap("./imagenes/eye_icon.ico")
